...
export function createReportRows(diffCoverage) {
  return diffCoverage.files.map((file) => {
    ...
    const percentage = executableLineCount === 0 ? 100 : (file.coveredLines.length / executableLineCount) * 100;

    return {
      File: file.filePath,
      'Changed Lines': file.changedLines.length,
      'Covered Lines': file.coveredLines.length,
      Percentage: executableLineCount === 0 ? '100% (No executable changes)' : `${formatPercentage(percentage)}%`,
    };
  });
}
...
export async function run(args, lifecycle = process) {
  try {
    ...
    const base = cliBase ?? env.baseBranch ?? config.baseBranch ?? (await getRemoteDefaultBranch(config.gitTimeoutMs));
